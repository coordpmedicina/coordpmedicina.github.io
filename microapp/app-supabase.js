...
function saveCompleteVersion() {
    ...
    let versions = [];
    try { versions = JSON.parse(localStorage.getItem(vKey) || '[]'); } catch (e) {}
...
function generatePreview() {
    ...
    const ch = (n, title) => `
        <h1 id="bk-ch${n}" class="bk-chapter">
            <span class="bk-ch-num">${n}</span> ${title}
        </h1>`;
    const sc = (chN, scN, title) => `
        <h2 id="bk-sc${chN}-${scN}" class="bk-section">
            <span class="bk-sc-num">${chN}.${scN}</span> ${title}
        </h2>`;
    ...
    const tocSections = {
        ...
        3: [['1','Misión y objetivos'],['2','Descripción y justificación']],
        4: [['1','Competencias por dimensión'],['2','Matriz de competencias'],['3','Resultados de Aprendizaje']],
    ...
    chapters.forEach((title, i) => {
        const n = i + 1;
        tocHtml += `<li><a href="#" onclick="document.getElementById('bk-ch${n}').scrollIntoView({behavior:'smooth',block:'start'});return false;">${n} ${title}</a>`;
        if (tocSections[n]) {
            tocHtml += `<ul>`;
            tocSections[n].forEach(([sn, st]) => {
                tocHtml += `<li><a href="#" onclick="document.getElementById('bk-sc${n}-${sn}').scrollIntoView({behavior:'smooth',block:'start'});return false;">${n}.${sn} ${st}</a></li>`;
            });
            tocHtml += `</ul>`;
        }
        tocHtml += `</li>`;
    ...
    .bk-ch-num {
        ...
        width: 34px; height: 34px; background: #1D5FA6; color: white;
    ...
    .bk-sc-num { color: #1D5FA6; margin-right: 4px; }
    .bk-subsection {
        font-family: -apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
    ...
    body += `<div class="bk-cover">
        ...
        <div class="bk-cover-title">${esc(id.subjectName)}</div>
        <div class="bk-cover-meta">Microcurrículo · ${new Date().toLocaleDateString('es-CO',{year:'numeric',month:'long',day:'numeric'})}</div>
        <div>
    ...
    body += `<div class="bk-dl">
        <span class="bk-dt">Nombre</span><span class="bk-dd">${na(id.subjectName)}</span>
        ...
        <span class="bk-dt">Componente curricular</span><span class="bk-dd">${na(id.component)}</span>
        <span class="bk-dt">Módulo</span><span class="bk-dd">${na(id.module)}</span>
    </div>`;
    body += sc(1,2,'Prerrequisitos');
    body += id.prerequisites?.length
        ? `<div class="bk-callout">${id.prerequisites.map(esc).join(' &nbsp;·&nbsp; ')}</div>`
    ...
    if (teachers.length) {
        body += bkTable(['Nombre','Email','Perfil profesional'],
            teachers.map(t => bkTr([esc(t.name), esc(t.email), esc(t.profile)])));
    } else { body += empty('Sin docentes registrados.'); }

    // ── Ch.3 Presentación ─────────────────────────────────────
    body += ch(3,'Presentación');
    body += sc(3,1,'Misión y objetivos');
    if (pres.departmentMission) body += `<p class="bk-subsection">Misión del departamento</p><div class="bk-text">${esc(pres.departmentMission)}</div>`;
    if (pres.generalObjectives) body += `<p class="bk-subsection">Objetivos generales</p><div class="bk-text">${esc(pres.generalObjectives)}</div>`;
    if (pres.specificObjectives) body += `<p class="bk-subsection">Objetivos específicos</p><div class="bk-text">${esc(pres.specificObjectives)}</div>`;
    if (!pres.departmentMission && !pres.generalObjectives && !pres.specificObjectives) body += empty('Sin información de misión y objetivos.');
    body += sc(3,2,'Descripción y justificación');
    if (pres.justification) body += `<p class="bk-subsection">Justificación</p><div class="bk-text">${esc(pres.justification)}</div>`;
    if (pres.generalDescription) body += `<p class="bk-subsection">Descripción general</p><div class="bk-text">${esc(pres.generalDescription)}</div>`;
    if (pres.advisorySpaces) body += `<p class="bk-subsection">Espacios de asesoría</p><div class="bk-text">${esc(pres.advisorySpaces)}</div>`;
    if (!pres.justification && !pres.generalDescription && !pres.advisorySpaces) body += empty('Sin descripción registrada.');

    // ── Ch.4 Competencias ─────────────────────────────────────
    body += ch(4,'Competencias');
    body += sc(4,1,'Competencias por dimensión');
    if (comp.ser || comp.saber || comp.saberHacer) {
        if (comp.ser)       body += `<p class="bk-subsection">Ser</p><div class="bk-text">${esc(comp.ser)}</div>`;
        if (comp.saber)     body += `<p class="bk-subsection">Saber</p><div class="bk-text">${esc(comp.saber)}</div>`;
        if (comp.saberHacer)body += `<p class="bk-subsection">Saber Hacer</p><div class="bk-text">${esc(comp.saberHacer)}</div>`;
    } else { body += empty('Sin competencias por dimensión registradas.'); }
    body += sc(4,2,'Matriz de competencias');
    if (matrix.length) {
        body += bkTable(['#','Competencia','Resultado de Aprendizaje','Estrategia metodológica'],
            matrix.map((m,i) => bkTr([i+1, esc(m.competencia), esc(m.resultado), esc(m.estrategia)])));
    } else { body += empty('Sin filas en la matriz de competencias.'); }
    body += sc(4,3,'Resultados de Aprendizaje');
    if (ras.length) {
        body += `<ol class="bk-ol">${ras.map(r=>`<li>${esc(r.text)}</li>`).join('')}</ol>`;
    } else { body += empty('Sin resultados de aprendizaje definidos.'); }
