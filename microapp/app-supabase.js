...
async function getOrCreateMcId(code) {
    ...
    if (!mcId) {
        ...
        try {
            ...
            if (p?.[0]?.id) attempts.push({ ...ins, created_by: null, program_id: p[0].id });
        } catch(_) {}
    ...
    try {
        ...
        if (existing?.[0]?.id) {
            ...
            const { error: insErr } = await supabase.from('microcurriculum_versions').insert({
                ...
                data: formData, status: 'autosave', created_by: null, notes: nowStr
            });
    ...
    try {
        ...
        for (const v of localVersions) {
            ...
            const { data: sbV, error: vErr } = await supabase.from('microcurriculum_versions').insert({
                ...
                data: v.data, status: 'draft', created_by: null, notes: v.date
            }).select('id').single();
...
async function saveCompleteVersion() {
    ...
    try {
        ...
        if (!mcId) {
            ...
            const { realName } = getExportMeta();
            const insertData = { subject_code: subjectCode, subject_name: realName, created_by: null };
