...
function renderEvaluationTable() {
    ...
    rows.push(`<tr style="border-top: 2px solid #1D5FA6;">
...
function renderEvalQualityPanel() {
    ...
    // Per-tiempo: sum of its momentos
    const subRows = table.map(t => {
        const sumMomentos = t.momentos.reduce((s, m) => s + (parseFloat(m.porcentaje) || 0), 0);
        const ok = Math.abs(sumMomentos - 100) < 0.01;
        const parts = t.momentos.map(m => `${m.nombre} ${m.porcentaje || '?'}`).join(' + ');
        return `<tr style="border-bottom: 1px solid #e8eef4;">
            <td style="padding: 8px 14px; font-size: 13px; color: #555;">
                Sub-total <strong>${t.nombre}</strong> (momentos)
            </td>
            <td style="padding: 8px 14px; font-size: 12px; color: #888;">${parts}</td>
            <td style="padding: 8px 14px; text-align: center; font-size: 13px;">
                ${fmt(sumMomentos)} &nbsp; ${badge(ok)}
            </td>
        </tr>`;
    }).join('');
