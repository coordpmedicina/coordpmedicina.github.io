...
function collectFormData() {
    ...
    formData.evaluation = {
        ...
        formative: getElementValue('formativeEvaluation'),
        firstExam: getElementValue('firstExam'),
        secondExam: getElementValue('secondExam'),
        finalExam: getElementValue('finalExam'),
        description: getElementValue('evaluationDescription'),
        criteria: getElementValue('gradingCriteria'),
    };
}
        ...
        const cells = days.map(day => {
            ...
            return `<td style="padding: 3px 6px; border-right: 1px solid #dce6ef;">
                <div contenteditable="true"
                     data-key="${key}"
                     style="min-height: 34px; padding: 6px 8px; border-radius: 4px; font-size: 12px; color: #333; cursor: text; border: 1px solid transparent; outline: none; transition: background 0.15s, border 0.15s;"
                     onfocus="this.style.background='#eef4ff'; this.style.borderColor='#2478C5';"
                     onblur="this.style.background=''; this.style.borderColor='transparent'; saveMallaCell(this.getAttribute('data-key'), this.textContent.trim())"
                >${val}</div>
            </td>`;
        }).join('');
...
function saveMallaCell(key, value) {
    if (!formData.mallaHoraria) formData.mallaHoraria = {};
    formData.mallaHoraria[key] = value;
    saveToLocalStorage();
}
