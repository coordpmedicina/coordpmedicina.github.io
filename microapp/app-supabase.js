...
async function syncAutosaveToSupabase() {
    ...
    if (!code) return;
    try {
        // Encontrar o crear microcurriculum_id
        let mcId = currentMicrocurriculumId;
        if (!mcId) {
            const { data: mcRows } = await supabase.from('microcurriculums').select('id').eq('subject_code', code).limit(1);
            mcId = mcRows?.[0]?.id;
        }
        if (!mcId) {
            const { realName } = getExportMeta();
            const ins = { subject_code: code, subject_name: realName, created_by: 'anonymous' };
            try { const { data: p } = await supabase.from('programs').select('id').limit(1); if (p?.[0]?.id) ins.program_id = p[0].id; } catch(_) {}
            const { data: nm, error: nmErr } = await supabase.from('microcurriculums').insert(ins).select('id').single();
            if (nmErr) return;
            mcId = nm.id;
        }
        // Buscar autosave existente para esta asignatura
        ...
        if (existing?.[0]?.id) {
            await supabase.from('microcurriculum_versions')
                .update({ data: formData, notes: nowStr })
                .eq('id', existing[0].id);
        } else {
            await supabase.from('microcurriculum_versions').insert({
                microcurriculum_id: mcId,
                version_number: 0,
                version_name: 'Autoguardado',
                data: formData,
                status: 'autosave',
                created_by: 'anonymous',
                notes: nowStr
        ...
        console.warn('Autosave Supabase silencioso:', e.message);
    }
}
