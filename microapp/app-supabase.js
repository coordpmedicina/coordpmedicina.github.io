...
function renderCompStrategiesCheckboxes() {
    ...
    if (teachingStrategies.length === 0) {
        container.innerHTML = '<p style="color: #999; font-size: 12px; grid-column: 1/-1;">No hay estrategias cargadas</p>';
        return;
    }
    container.innerHTML = teachingStrategies.map(s => `
        <label style="display: flex; align-items: flex-start; gap: 6px; cursor: pointer; padding: 5px 6px; border-radius: 4px; font-size: 12px; transition: background 0.15s;"
               onmouseover="this.style.background='#e8f0ff'" onmouseout="this.style.background='transparent'">
            <input type="checkbox" class="comp-estrategia-checkbox" value="${s.abbreviation}" data-name="${s.name}" style="margin-top: 2px; accent-color: #1D5FA6; flex-shrink: 0;">
            <span><strong style="color: #1D5FA6;">${s.abbreviation}</strong> — ${s.name}</span>
        </label>
    `).join('');
}
...
function renderMethodStrategyCheckboxes() {
    ...
    container.innerHTML = teachingStrategies.map(s => `
        <label style="display: flex; align-items: flex-start; gap: 6px; cursor: pointer; padding: 5px 6px; border-radius: 4px; font-size: 12px; transition: background 0.15s;"
               onmouseover="this.style.background='#e8f0ff'" onmouseout="this.style.background='transparent'">
            <input type="checkbox" class="method-estrategia-checkbox" value="${s.abbreviation}" data-name="${s.name}" style="margin-top: 2px; accent-color: #1D5FA6; flex-shrink: 0;">
            <span><strong style="color: #1D5FA6;">${s.abbreviation}</strong> — ${s.name}</span>
        </label>
    `).join('');
}
...
function renderMallaHoraria() {
    ...
    if (!id?.subjectName) {
        container.innerHTML = '<p style="color: #999; font-size: 13px; padding: 20px; text-align: center;">Selecciona una asignatura para ver su información de carga horaria.</p>';
        return;
    }

    const hours = id.directTeachingHours || id.hoursPerWeek || '—';
    const credits = id.credits || '—';
    const semester = id.semester || '—';

    container.innerHTML = `
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 16px; margin-bottom: 16px;">
            <div style="background: linear-gradient(135deg, #1D5FA6, #2478C5); border-radius: 10px; padding: 20px; color: white; text-align: center;">
                <div style="font-size: 36px; font-weight: 700; line-height: 1;">${hours}</div>
                <div style="font-size: 13px; margin-top: 6px; opacity: 0.9;">Horas docencia directa / semana</div>
            </div>
            <div style="background: #f0f4f8; border: 1px solid #dce6ef; border-radius: 10px; padding: 20px; text-align: center;">
                <div style="font-size: 36px; font-weight: 700; color: #1D5FA6; line-height: 1;">${credits}</div>
                <div style="font-size: 13px; color: #555; margin-top: 6px;">Créditos académicos</div>
            </div>
            <div style="background: #f0f4f8; border: 1px solid #dce6ef; border-radius: 10px; padding: 20px; text-align: center;">
                <div style="font-size: 36px; font-weight: 700; color: #1D5FA6; line-height: 1;">${semester}</div>
                <div style="font-size: 13px; color: #555; margin-top: 6px;">Semestre</div>
            </div>
        </div>
        <div style="background: #fff8e1; border: 1px solid #ffe082; border-radius: 8px; padding: 12px 16px; font-size: 12px; color: #7a5c00; display: flex; align-items: center; gap: 8px;">
            <span style="font-size: 16px;">ℹ️</span>
            <span>La asignación de horarios específicos (día y franja horaria) se coordina con la Oficina de Registro Académico. Esta información refleja la carga oficial del programa.</span>
        </div>`;
}
}
