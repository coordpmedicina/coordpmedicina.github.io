...
// Devuelve el microcurriculum_id para un subject_code, creándolo si no existe.
// Retorna null y loguea el error si falla.
async function getOrCreateMcId(code) {
    let mcId = currentMicrocurriculumId;
    if (!mcId) {
        const { data: rows, error: findErr } = await supabase
            .from('microcurriculums').select('id').eq('subject_code', code).limit(1);
        if (findErr) { console.error('mcId find error:', findErr.message); return null; }
        mcId = rows?.[0]?.id || null;
    }
    if (!mcId) {
        const { realName } = getExportMeta();
        // Intentar insertar con campos mínimos; si falla con NOT NULL, agregar más
        let ins = { subject_code: code, subject_name: realName };
        const attempts = [
            ins,
            { ...ins, created_by: 'anonymous' },
        ];
        // Intentar con program_id si existe tabla programs
        try {
            const { data: p } = await supabase.from('programs').select('id').limit(1);
            if (p?.[0]?.id) attempts.push({ ...ins, created_by: 'anonymous', program_id: p[0].id });
        } catch(_) {}

        for (const payload of attempts) {
            const { data: nm, error: nmErr } = await supabase
                .from('microcurriculums').insert(payload).select('id').single();
            if (!nmErr) { mcId = nm.id; break; }
            console.warn('mcId insert attempt failed:', nmErr.message, 'payload:', JSON.stringify(payload));
        }
        if (!mcId) {
            console.error('No se pudo crear microcurriculum en Supabase para', code);
            return null;
        }
    }
    return mcId;
...
async function syncAutosaveToSupabase() {
    ...
    if (!code) return;
    try {
        const mcId = await getOrCreateMcId(code);
        if (!mcId) return;

        const nowStr = new Date().toLocaleString('es-CO');
        const { data: existing } = await supabase
            .from('microcurriculum_versions').select('id')
            .eq('microcurriculum_id', mcId).eq('status', 'autosave').limit(1);

        if (existing?.[0]?.id) {
            const { error: upErr } = await supabase.from('microcurriculum_versions')
                .update({ data: formData, notes: nowStr }).eq('id', existing[0].id);
            if (upErr) console.error('Autosave update error:', upErr.message);
        } else {
            const { error: insErr } = await supabase.from('microcurriculum_versions').insert({
                microcurriculum_id: mcId, version_number: 0, version_name: 'Autoguardado',
                data: formData, status: 'autosave', created_by: 'anonymous', notes: nowStr
            });
...
async function forceSyncToSupabase() {
    ...
    try {
        const mcId = await getOrCreateMcId(code);
        if (!mcId) {
            showAlert('❌ No se pudo crear/encontrar el registro en Supabase. Revisa la consola para el error exacto.', 'error');
            return;
        }

        // 1. Guardar estado actual como autosave
        const nowStr = new Date().toLocaleString('es-CO');
        const { data: exAS } = await supabase.from('microcurriculum_versions')
            .select('id').eq('microcurriculum_id', mcId).eq('status', 'autosave').limit(1);
        let autosaveOk = false;
        if (exAS?.[0]?.id) {
            const { error: e } = await supabase.from('microcurriculum_versions')
                .update({ data: formData, notes: nowStr }).eq('id', exAS[0].id);
            autosaveOk = !e;
            if (e) console.error('Autosave update:', e.message);
        } else {
            const { error: e } = await supabase.from('microcurriculum_versions').insert({
                microcurriculum_id: mcId, version_number: 0, version_name: 'Autoguardado',
                data: formData, status: 'autosave', created_by: 'anonymous', notes: nowStr
            });
            autosaveOk = !e;
        ...
        for (const v of localVersions) {
            ...
            const { data: prev } = await supabase.from('microcurriculum_versions')
                .select('version_number').eq('microcurriculum_id', mcId)
                .neq('status', 'autosave').order('version_number', { ascending: false }).limit(1);
            const nextVn = (prev?.[0]?.version_number || 0) + 1;
            const { data: sbV, error: vErr } = await supabase.from('microcurriculum_versions').insert({
                microcurriculum_id: mcId, version_number: nextVn, version_name: v.name,
                data: v.data, status: 'draft', created_by: 'anonymous', notes: v.date
            }).select('id').single();
...
async function loadSubjectDetails() {
    ...
    try {
        ...
        if (savedRaw) {
            ...
            if (supabaseReady) {
                try {
                    const { data: mcRows } = await supabase.from('microcurriculums').select('id').eq('subject_code', subject.code).limit(1);
                    if (mcRows?.[0]?.id) {
                        const { data: sbV } = await supabase
                            .from('microcurriculum_versions')
                            .select('id, data, status, version_name, created_at')
                            .eq('microcurriculum_id', mcRows[0].id)
                            .order('created_at', { ascending: false })
                            .limit(10);
                        // Preferir autosave; si no, la versión más reciente
                        const best = sbV?.find(v => v.status === 'autosave') || sbV?.[0];
                        if (best?.data) {
                            formData = best.data;
                            formData.identification = { ...formData.identification, ...freshIdentification };
                            loadFormFromData();
...
async function saveCompleteVersion() {
    ...
    try {
        // Buscar microcurriculum_id: primero usar el actual si está cargado,
        // si no, buscar por subject_code en la tabla existente
        let mcId = currentMicrocurriculumId;
        if (!mcId) {
            ...
            mcId = mcRows?.[0]?.id || null;
        }

        if (!mcId) {
            // No existe aún — crear el registro automáticamente
            const { realName } = getExportMeta();
            const insertData = { subject_code: subjectCode, subject_name: realName, created_by: 'anonymous' };
            // Intentar obtener program_id si la tabla lo requiere
            try {
                const { data: programs } = await supabase.from('programs').select('id').limit(1);
                if (programs?.[0]?.id) insertData.program_id = programs[0].id;
            } catch(_) {}
            const { data: newMc, error: mcInsertErr } = await supabase
                .from('microcurriculums')
                .insert(insertData)
                .select('id')
                .single();
            if (mcInsertErr) throw mcInsertErr;
            mcId = newMc.id;
        }
        ...
        const { data: prevVersions } = await supabase
            .from('microcurriculum_versions')
            .select('version_number')
            .eq('microcurriculum_id', mcId)
            .order('version_number', { ascending: false })
            .limit(1);
        const nextVersion = (prevVersions?.[0]?.version_number || 0) + 1;

        // Insertar versión en Supabase
        const { data: sbV, error: vErr } = await supabase
            .from('microcurriculum_versions')
            .insert({
                microcurriculum_id: mcId,
                version_number: nextVersion,
                version_name: versionName,
                data: formData,
                status: 'draft',
        ...
        // Actualizar entrada local con el ID de Supabase
        entry.sbId = sbV.id;
        ...
        try {
            ...
            if (mcRows && mcRows.length > 0) {
                ...
                const { data: sbVersions } = await supabase
                    .from('microcurriculum_versions')
                    .select('id, version_name, notes, created_at, data, version_number')
                    .eq('microcurriculum_id', mcId)
                    .neq('status', 'autosave')
                    .order('created_at', { ascending: false });
                if (sbVersions && sbVersions.length > 0) {
                    ...
                    // Add local-only versions not yet in Supabase
                    ...
                    // Persist Supabase versions to localStorage for offline fallback
