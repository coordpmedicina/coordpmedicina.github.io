...
async function saveCompleteVersion() {
    ...
    // Also save to Supabase when available
    if (supabaseReady) {
        try {
            const { realName } = getExportMeta();
            // Find or create microcurriculum record for this subject
            let mcId;
            const { data: existingMc } = await supabase
                ...
                .limit(1);
            if (existingMc && existingMc.length > 0) {
                mcId = existingMc[0].id;
            } else {
                const { data: newMc, error: mcErr } = await supabase
                    .from('microcurriculums')
                    .insert({ subject_code: subjectCode, subject_name: realName })
                    .select('id')
                    .single();
