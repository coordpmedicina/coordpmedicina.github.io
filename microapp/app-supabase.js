...
function deleteEvalMomento(tiempoId, momentoId) {
    ...
    if (!t || t.momentos.length <= 1) {
        showAlert('Debe haber al menos un momento evaluativo por tiempo', 'warning');
        return;
    }
    if (!confirm('¿Eliminar este momento evaluativo y todos sus tipos?')) return;
