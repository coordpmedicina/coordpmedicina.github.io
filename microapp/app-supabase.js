...
function buildMarkdownBody(fd, realName, realModule, prereqItems) {
    ...
    // 1. Identificación
    md += `# 1 Identificación\n\n`;
    md += `## 1.1 Datos de identificación\n\n`;
    md += `| Campo | Valor |\n|---|---|\n`;
    ...
    md += `\n# 5 Metodología y Contenidos\n\n`;
    md += `## 5.1 Temas y estrategias pedagógicas\n\n`;
    if (topics.length) {
    ...
    // 7. Cronograma
    md += `\n# 7 Cronograma\n\n`;
    md += `## 7.1 Distribución semanal de temas\n\n`;
    if (topics.length) {
    ...
    md += `\n## 7.2 Distribución semanal de rotaciones\n\n`;
    ...
    printWindow.document.write(`<!DOCTYPE html><html lang="es"><head>
        ...
        <style>
            ...
            * { box-sizing: border-box; }
            ...
            .bk-chapter { font-size: 18px; page-break-before: always; }
            .bk-chapter:first-of-type { page-break-before: avoid; }
            ...
            @media print {
                ...
                .bk-chapter { page-break-before: always; }
                .bk-chapter:first-of-type { page-break-before: avoid; }
...
function parseAndLoadMarkdown(content) {
    ...
    // Utility: extract block content under a heading (## N.M) until next ## or #
    ...
    // Utility: parse ### subsections within a block
    ...
    // Fallback: extract code from cover blockquote
    if (!newData.identification.subjectCode) {
        const m = content.match(/Código:\s*([A-Z]+-\d+)/i);
    ...
    // ── 3.1 Presentación ──
    const sec31 = getBlock(content, '## 3.1');
    const presMap = {
    ...
    parseMdTable(sec71).slice(1).forEach(row => {
        const topic = newData.methodology.find(t => t.topic === row[0]);
        if (topic) newData.cronograma.push({ topicId: topic.id, semana: row[1] || '', fechas: row[2] || '', observaciones: row[3] || '' });
    });
    if (newData.cronograma.length) importLog.push('✓ Cronograma');
    ...
    container.innerHTML = versions.slice().reverse().map(v => `
        <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;background:#f7fafd;border:1px solid #dce6ef;border-radius:8px;margin-bottom:8px;">
            <div>
                ...
                <div style="font-size:11px;color:#888;margin-top:2px;">📅 ${v.date}</div>
            </div>
...
function restoreVersion(versionId) {
    ...
    if (!version) { showAlert('Versión no encontrada', 'error'); return; }

    const confirmed = confirm(
        `⚠️ ¿Restaurar la versión "${version.name}" (${version.date})?\n\n` +
    ...
    loadFormFromData();
    saveLocalStorageSilent();
    showAlert(`✅ Versión "${version.name}" restaurada correctamente`, 'success');
    renderSavedVersions();
}
